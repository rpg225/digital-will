...
const normalizeWill = (w) => {
  ...
  const amounts = Array.isArray(rawAmounts) ? rawAmounts : rawAmounts ? [rawAmounts] : [];
...
const useGetWills = () => {
  ...
  // ✅ TWO FLAGS (DO NOT REUSE ONE BOOLEAN)
  const [hasActiveWill, setHasActiveWill] = useState(false);
  const [hasEverCreatedWill, setHasEverCreatedWill] = useState(false);
  ...
  const fetchAllWills = useCallback(async () => {
    ...
    try {
      ...
      const events = await filterInstance.queryFilter(filterInstance.filters.WillCreated(), 0, "latest");

      setWillsCreated(events.length);
      setHasEverCreatedWill(events.length > 0);

      setTotalBalance(
        events.reduce((acc, e) => acc + parseFloat(ethers.utils.formatEther(e.args.balance)), 0)
      );

      // ---------- USER WILL (ACTIVE STATE) ----------
      ...
      for (const addr of uniqueTestators) {
        try {
          ...
          const timeLeft = will.lastPing + will.deathTimeout - now;

          willList.push({
            address: addr,
            balanceEth: ethers.utils.formatEther(will.balance),
            beneficiaries: will.beneficiaries,
            amounts: will.amounts.map((a) => ethers.utils.formatEther(a)),
